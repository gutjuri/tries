...
prepare_enable1_trie(std::vector<std::pair<std::string, std::size_t>> &v) {
  std::map<std::string, std::size_t> trie;
  for (auto p : v) {
...
TEST_CASE("Make map from vector") {
  auto v = read_enable1();
  BENCHMARK("Convert vector to map") { return prepare_enable1_trie(v); };
}

TEST_CASE("Query large map") {
  auto v = read_enable1();
  std::map<std::string, std::size_t> trie = prepare_enable1_trie(v);
